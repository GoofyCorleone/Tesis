...
\section{Marco Teórico}
	...
	\subsection{Cuantización del campo electromagnético}
		...
		Para cuantizar el campo electromagnético en una cavidad de volumen \(V\), partimos de la descripción del potencial vectorial bajo el gauge de Lorentz, donde tanto el potencial escalar como el vectorial satisfacen la ecuación de onda en ausencia de fuentes. Al imponer la condición de transversalidad, adoptamos el gauge de Coulomb, lo que permite expresar el potencial vectorial como una expansión en modos armónicos. Cada modo está caracterizado por un vector de onda \(\vec{k}\), su frecuencia \(\omega_{\mathbf{k}}\) y una polarización \(\vec{e}_{\mathbf{k}}\). A partir de este potencial, los campos eléctrico y magnético se obtienen mediante derivadas temporales y espaciales, respectivamente.
		
		El Hamiltoniano del campo en la cavidad se escribe inicialmente en términos de los operadores de creación y destrucción asociados a cada modo, resultando en una suma sobre energías \(\hbar\omega_{\mathbf{k}}\). Para una formulación más conveniente, se introduce la representación en variables canónicas \((q_{\mathbf{k}}, p_{\mathbf{k}})\), lo que transforma el Hamiltoniano en una suma de osciladores armónicos independientes:
		\[
		H = \frac{1}{2}\sum_{\mathbf{k}}\left[p_{\mathbf{k}}^2 + \omega_{\mathbf{k}}^2 q_{\mathbf{k}}^2\right].
		\]
		Este resultado muestra que la cuantización del campo electromagnético equivale a la cuantización de un conjunto de osciladores armónicos, base fundamental para la teoría cuántica de la radiación.
		...
		\end{equation}
		y usando los operadores de Campo eléctrico y campo magnético se expresan de la siguiente forma
...
\section{Cronograma}
	Se presenta el cronograma en la tabla \ref{tab:cronograma_resumen} en el cual se expresan con mucho detalle el calendario de actividades que se planea abordar.
...
\section{Presupuesto}
	En esta parte se presenta en la tabla \ref{tab:presupuesto} los gastos necesarios para realizar esta investigación.
	
	\begin{table}[h!]
		\centering
		\caption{Presupuesto global}
		\label{tab:presupuesto}
		\begin{tabular}{|p{2.5cm}|p{4.5cm}|r|r|}
			\hline
			\textbf{Rubros} & \textbf{Detalle} & \multicolumn{2}{c|}{\textbf{Aportes (Especie)}} \\
			\cline{3-4}
			& & \textbf{GOTS} & \textbf{UIS} \\
			\hline
			Equipos & Uso workstation & 6.325.000 & -- \\
			\hline
			Talento Humano & Director & -- & 29.280.000 \\
			\cline{2-4}
			& Estudiante de maestría & -- & 15.840.000 \\
			\hline
			Recursos Electrónicos & Computador portátil & -- & 9.000.000 \\
			\hline
			Participación en eventos científicos & Asistencia a eventos internacionales & -- & 1.500.000 \\
			\hline
			\hline
			\textbf{Totales} & & \textbf{6.325.000} & \textbf{55.620.000} \\
			\hline
			\multicolumn{3}{|r|}{\textbf{Total General}} & \textbf{61.945.000} \\
			\hline
		\end{tabular}
	\end{table}
